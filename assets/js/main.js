...
const CodeMatrixAnimation = {
    canvas: null,
    ctx: null,
    animationId: null,
    drops: [],
    scripts: [
        "import torch", "model.train()", "optim.step()", "docker push",
        "kubectl apply", "mlflow.log()", "git commit", "lambda x: x**2",
        "npm start", "pip install", "loss.backward()", "def forward(x):",
        "plt.show()", "pandas.read_csv", "tf.keras.layers", "dist.init()",
        "PCR()", "DNA.seq()", "CRISPR.edit()", "protein.fold()"
    ],

    init() {
        this.canvas = document.getElementById('codeMatrix');
    ...
    animate() {
        ...
        // Fade effect
        this.ctx.fillStyle = 'rgba(1, 4, 9, 0.1)';
        this.ctx.fillRect(0, 0, this.canvas.width, this.canvas.height);

        this.ctx.font = `${this.fontSize}px monospace`;

        for (let i = 0; i < this.drops.length; i++) {
            const text = this.scripts[Math.floor(Math.random() * this.scripts.length)];
            const char = text[Math.floor(Math.random() * text.length)];

            // Alternate between emerald and blue
            const isEmerald = Math.random() > 0.5;
            this.ctx.fillStyle = isEmerald
...
// === ENHANCED SCROLL HANDLER FOR NAVIGATION ===
...
// Initialize code matrix and scroll handler on DOMContentLoaded
document.addEventListener('DOMContentLoaded', function () {
    CodeMatrixAnimation.init();
    EnhancedScrollHandler.init();
});
